...
class MockLLM:
    ...
    def complete(
        ...
        *,
        ...
        self._completion_history.append(dict(self._last_completion_metadata))
        if "PAIRWISE_TEXTBOOK_JUDGE" in prompt:
            dimensions = {
                "goal_alignment": "A",
                "curriculum_coherence": "A",
                "chapter_dependency_quality": "A",
                "teaching_clarity": "Tie",
                "material_faithfulness": "A",
                "factual_accuracy": "A",
                "editability": "A",
                "formatting_reliability": "A",
            }
            return json.dumps(
                {
                    "winner": "A",
                    ...
                    "dimension_evidence": {
                        item: {"A": "Mock evidence for A.", "B": "Mock evidence for B."}
                        for item in dimensions
                    },
                    "rationale": "Mock judgment prefers output A for contract alignment.",
                },
                ensure_ascii=False,
            )
        if "VALIDATE_REPAIR_JSON" in prompt or "RETRY_REPAIR_VALIDATION_SCHEMA_JSON" in prompt:
            issue_ids = _mock_validation_issue_ids(prompt)
            return json.dumps(
                {
                    "passed": True,
                    "resolutions": [
                        {"issue_id": issue_id, "status": "resolved", "evidence": "Mock validation resolved it."}
                        for issue_id in issue_ids
                    ],
                    "new_issues": [],
                    "summary": "Mock repair validation passed.",
                },
                ensure_ascii=False,
            )
        if "REVIEW_CHAPTER_JSON" in prompt:
            return json.dumps({"passed": True, "issues": [], "summary": "Mock review passed."}, ensure_ascii=False)
        if "REPAIR_CHAPTER_MARKDOWN" in prompt:
            marker = "CURRENT_CHAPTER_MARKDOWN:"
            current = prompt.split(marker, 1)[-1].strip() if marker in prompt else "# Repaired Chapter"
            return current + "\n\n## Mock revision\n\nThis deterministic revision addresses the recorded issues."
        if "PLAN_WITH_CONTRACTS_JSON" in prompt:
            return json.dumps(mock_contract_plan(_prompt_integer(prompt, "CHAPTER_COUNT", 2)), ensure_ascii=False)
        if "SINGLE_AGENT_PLAN_JSON" in prompt:
            chapter_count = 2
            for line in prompt.splitlines():
                if line.startswith("CHAPTER_COUNT:"):
                    chapter_count = int(line.split(":", 1)[1].strip())
                    break
            return json.dumps(mock_single_agent_outline(chapter_count), ensure_ascii=False)
        if "SINGLE_AGENT_WRITE_CHAPTER_MARKDOWN" in prompt:
            ...
            for line in prompt.splitlines():
                if line.startswith("CHAPTER_TITLE:"):
                    ...
                    break
            return f"# {title}\n\nThis mock single-author chapter follows the ordinary outline and explains the requested topic.\n\n## Summary\n\nThe chapter prepares the next step.\n\n## Exercises\n\n1. Explain the key idea.\n2. Apply it to a small example."
        if "DIRECT_TEXTBOOK_MARKDOWN" in prompt:
            return "# Direct Prompt Textbook\n\n## Chapter 1: Overview\n\nThis is a mock direct-prompt textbook chapter."
        if "WRITE_CHAPTER_MARKDOWN" in prompt:
            ...
            for line in prompt.splitlines():
                if line.startswith("CHAPTER_TITLE:"):
                    title = line.split(":", 1)[1].strip()
                    break
            return mock_chapter_markdown(
                title,
                minimum_characters=_prompt_integer(prompt, "CHAPTER_MIN_CHARACTERS", 0),
            )
        return "{}"
...
def mock_contract_plan(chapter_count: int = 2) -> dict[str, Any]:
    chapters = [
        {
            "title": "Foundations",
            "description": "Introduce the central problem and prerequisite concepts.",
            "purpose": "Build shared vocabulary.",
            "connection_from_previous": "Course entry point.",
            "setup_for_next": "Prepare the formal model.",
            "depth": "normal",
            "time": {"reading_minutes": 90, "exercise_minutes": 45, "practice_minutes": 45, "extension_minutes": 20},
            "contract": {
                "chapter_title": "Foundations",
                "required_topics": ["problem framing", "basic definitions"],
                "introduced_concepts": ["problem framing", "basic definitions"],
                "prerequisite_concepts": [],
                "bridge_from_previous": "Start from learner background.",
                "bridge_to_next": "Introduce formal notation.",
                "forbidden_early_topics": ["advanced algorithms"],
                "required_examples": ["one running example"],
                "required_formulas": [],
                "prepares_for": ["formal notation"],
                "assessment_targets": ["explain the problem framing"],
                "summary_for_next": "The learner now understands the problem framing.",
            },
        },
        {
            "title": "Formal Model",
            "description": "Develop the mathematical model and core equations.",
            "purpose": "Make the curriculum precise.",
            "connection_from_previous": "Uses definitions from Foundations.",
            "setup_for_next": "Prepare computation and algorithms.",
            "depth": "core",
            "time": {"reading_minutes": 120, "exercise_minutes": 60, "practice_minutes": 60, "extension_minutes": 30},
            "contract": {
                "chapter_title": "Formal Model",
                "required_topics": ["state", "action", "transition", "reward"],
                "introduced_concepts": ["state", "action", "transition", "reward"],
                "prerequisite_concepts": ["problem framing", "basic definitions"],
                "bridge_from_previous": "Formalize previous intuitions.",
                "bridge_to_next": "Show why algorithms are needed.",
                "forbidden_early_topics": ["deep learning extensions"],
                "required_examples": ["grid-world style example"],
                "required_formulas": ["Bellman-style recursion"],
                "prepares_for": ["algorithms"],
                "assessment_targets": ["write the formal model of a small decision problem"],
                "summary_for_next": "The learner can describe the model mathematically.",
            },
        },
    ]
    while len(chapters) < chapter_count:
        index = len(chapters) + 1
        previous_title = chapters[-1]["title"]
        title = f"Application {index}"
        chapters.append(
            {
                "title": title,
                "description": "Apply the formal model in a worked example.",
                "purpose": "Develop practical fluency.",
                "connection_from_previous": f"Uses {previous_title}.",
                "setup_for_next": "Prepare the next application." if index < chapter_count else "Conclude the course.",
                "depth": "normal",
                "time": {"reading_minutes": 90, "exercise_minutes": 45, "practice_minutes": 45, "extension_minutes": 20},
                "contract": {
                    "chapter_title": title,
                    "required_topics": [f"application {index}"],
                    "introduced_concepts": [f"application {index}"],
                    "prerequisite_concepts": ["basic definitions"],
                    "bridge_from_previous": f"Uses {previous_title}.",
                    "bridge_to_next": "Prepare the next application." if index < chapter_count else "Conclude the course.",
                    "forbidden_early_topics": [],
                    "required_examples": ["one worked application"],
                    "required_formulas": [],
                    "prepares_for": [],
                    "assessment_targets": ["apply the model"],
                    "summary_for_next": "The learner can apply the model.",
                },
            }
        )
    ...
    dependencies = [
        {
            ...
            "depends_on": [chapters[index - 1]["title"]] if index else [],
            "introduces": chapter["contract"]["introduced_concepts"],
            "prepares_for": [chapters[index + 1]["title"]] if index + 1 < len(chapters) else [],
        }
        for index, chapter in enumerate(chapters)
    ]
    return {
        "course_bible": {
            "target_learner": "Learners with basic mathematical background.",
            "final_outcomes": ["Understand the topic", "Apply the model to examples"],
            "teaching_style": "Clear, structured, example-driven.",
            "prerequisites": ["basic algebra", "probability"],
            "global_narrative": "Move from intuition to formalism and then application.",
            "terminology": [{"term": "contract", "definition": "chapter-level pedagogical constraint", "introduced_in": "Foundations"}],
...
def mock_chapter_markdown(title: str, *, minimum_characters: int = 0) -> str:
    introduction = (
        f"# {title}\n\nThis mock chapter follows its curriculum contract and explains the required topics.\n\n"
    )
    paragraph = (
        "This deterministic paragraph develops the concept, connects it to the running example, "
        "checks the reasoning, and prepares a concrete exercise for the learner. "
    )
    body = introduction
    while len(body) < minimum_characters:
        body += paragraph
    return (
        f"{body}\n\n## Summary\n\nThe chapter prepares the next step.\n\n"
        "## Exercises\n\n1. Explain the key idea.\n2. Apply it to a small example."
...
def _prompt_integer(prompt: str, label: str, default: int) -> int:
    for line in prompt.splitlines():
...
def _mock_validation_issue_ids(prompt: str) -> list[str]:
    if "EXPECTED_ORIGINAL_ISSUE_IDS_JSON:" in prompt:
        tail = prompt.split("EXPECTED_ORIGINAL_ISSUE_IDS_JSON:", 1)[1].lstrip()
        ...
        try:
            ...
            pass
    if "ORIGINAL_REVIEW_ISSUES_JSON:" in prompt:
        block = prompt.split("ORIGINAL_REVIEW_ISSUES_JSON:", 1)[1].split("Output schema:", 1)[0]
        return list(dict.fromkeys(re.findall(r'"id"\s*:\s*"([RDN]\d+)"', block)))
    return []


def mock_single_agent_outline(chapter_count: int = 2) -> dict[str, Any]:
    chapters = []
    for index in range(chapter_count):
        title = "Overview" if index == 0 else f"Application {index}"
        chapters.append(
            {
                "title": title,
                "description": "A broad introduction." if index == 0 else "Apply the core ideas in a worked example.",
                "purpose": "Establish the central ideas." if index == 0 else "Develop practical fluency.",
                "connection_from_previous": "Course entry point." if index == 0 else "Uses the concepts from the overview.",
                "setup_for_next": "Prepare the formal model." if index < chapter_count - 1 else "Course conclusion.",
                "depth": "normal",
                "time": {"reading_minutes": 90, "exercise_minutes": 45, "practice_minutes": 45, "extension_minutes": 20},
            }
        )
    return {
        "course_overview": "A short mock textbook outline.",
        "chapters": chapters,
    }
